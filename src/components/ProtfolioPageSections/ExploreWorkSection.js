...
const ExploreWorkSection = ({data,totalCount,seeMore}) => {
  ...
  const markets = useSelector((state) => state.home.marketData);
  ...
  useEffect(() => {
    ...
    dispatch(fetchMarketTopsections());
  }, [dispatch]);
                          ...
                          <li key={index}>
                            <button
                             onClick={()=>{handleStudioFilter(item.data.cardName)}}
                              className={`portfolio-btn-tag ${
                                selectedStudio.includes(item.data.cardName)
                                  ? "active"
                                  : ""
                              }`}>
                              {item.data.cardName}
                    ...
                    <div className="wrapper-list">
                      <ul className="list-market-tags list-dropdown-tags">
                        ...
                        {markets?.map((market, index) => (
                          <li key={index}>
                            <button
                             onClick={()=>{handleMarketFilter(market.data.cardname)}}
                              className={`portfolio-btn-tag ${
                                selectedMarkets.includes(market.data.cardname)
                                  ? "active"
                                  : ""
                              }`}
                              key={`category-${index}`}
                            >
                              {market.data.cardname}
                            </button>
                          </li>
                        ))}
                      </ul>
                      ...
                      <div className="container-text">
                        <ul className="list-tags-small">
                          {item.markets.map((market, index) => (
                            <li key={index} onClick={()=>{handleMarketFilter(market.cardname)}} className={`tag-small ${
                              selectedMarkets.includes(market.cardname)
                                ? "active"
                                : ""
                            }`}  >
                          ...
                          {item.studios.map((studio, index) => (
                            <React.Fragment key={index}>
                              {index < 2 && (
                                <li onClick={()=>{handleStudioFilter(studio.cardName)}} className={`tag-small ${
                                  selectedStudio.includes(studio.cardName)
                                    ? "active"
                                    : ""
                                }`} >
